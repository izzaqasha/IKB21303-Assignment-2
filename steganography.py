...
def hide_file(cover_image_path, secret_file_path, output_image_path):
    ...
    print(f"Secret filename: {filename}")
    print(f"Secret file size: {file_size} bytes")
    print(f"First 20 secret bytes: {secret_data[:20]}\n")
    ...
    print(f"Header: {payload[:4]}")
    print(f"Filename length bytes: {payload[4:6]}")
    print(f"File size bytes: {payload[6:14]}")
    print(f"Payload first 30 bytes: {payload[:30]}\n")
    ...
    print(f"Bits actually embedded: {bit_index}\n")

    ## Modify pixels back to the image
    image.putdata(new_pixels)

    ## Save the modified image with standard lossless compression and optimization enabled
...
def extract_file(stego_image_path, output_directory):
    ...
    print(f"Maximum extractable bits: {len(pixels) * 3}\n")
    ...
    print(f"Extracted total bytes: {len(data)}")
    print(f"First 30 extracted bytes: {data[:30]}")

    if len(data) < 14: # if the length of the extracted data is less than 14 bytes, it means that there is not enough data to contain the header, filename length, and file size information.
        raise ValueError("The image does not contain enough hidden data.")

    print(f"Extracted header: {data[:4]}\n")

    ## Check for the header to ensure that the image contains hidden data.
    if data[:4] != b"STEG": # If the first 4 bytes of the extracted data do not match the expected header "STEG", it indicates that the image does not contain valid hidden data, and an error is raised. This check helps to verify that the extraction process is working correctly and that the image being processed actually contains hidden information.
    ...
    if filename_end > len(data): # If the calculated end index for the filename exceeds the length of the extracted data, it indicates that the filename information is invalid or corrupted.
    ...
    print(f"Extracted secret data length: {len(secret_data)}")
    print(f"First 20 extracted secret bytes: {secret_data[:20]}\n")
